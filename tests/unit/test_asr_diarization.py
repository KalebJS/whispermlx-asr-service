...
class TestDiarizeToggle:
    ...
    def test_diarize_true_produces_speaker_labels(self, client):
        ...
        assert len(speakers) >= 2, f"Expected >=2 distinct speakers with diarize=true, got: {speakers}"
    ...
    def test_diarize_false_omits_speakers(self, client):
        ...
        for seg in body["segments"]:
            assert not seg.get("speaker"), f"Segment should not have speaker label with diarize=false: {seg}"
    ...
    def test_diarization_defaults_on(self, client):
        ...
        body = resp.json()
        speakers = {seg.get("speaker") for seg in body["segments"] if seg.get("speaker")}
        assert len(speakers) >= 1, "Diarization should default on; at least one speaker label expected"
    ...
    def test_enable_diarization_true_same_as_diarize_true(self, client):
        ...
        body = resp.json()
        speakers = {seg.get("speaker") for seg in body["segments"] if seg.get("speaker")}
        assert len(speakers) >= 2, f"enable_diarization=true should produce speaker labels, got: {speakers}"
...
class TestSpeakerLabelsInResponse:
    ...
    def test_words_have_speaker_labels(self, client):
        ...
        assert len(word_speakers) >= 1, f"Expected at least some words with speaker labels, got: {word_speakers}"
...
class TestSpeakerLabelsInFormats:
    ...
    def test_srt_includes_speaker_labels(self, client):
        ...
        assert "[SPEAKER_" in srt_content, f"SRT output should contain [SPEAKER_NN] prefixes: {srt_content[:200]}"
    ...
    def test_vtt_includes_speaker_labels(self, client):
        ...
        assert "[SPEAKER_" in vtt_content, f"VTT output should contain [SPEAKER_NN] prefixes: {vtt_content[:200]}"
    ...
    def test_tsv_includes_speaker_column(self, client):
        ...
        for line in lines[1:]:
            ...
            assert cols[3].startswith("SPEAKER_"), f"Speaker column should have SPEAKER_NN label, got: {cols[3]}"
...
class TestSpeakerCountParams:
    ...
    def test_min_max_speakers_passed_to_pipeline(self, client):
        ...
        assert resp.status_code == 200
        assert captured_kwargs.get("min_speakers") == 2, f"min_speakers=2 not passed: {captured_kwargs}"
        assert captured_kwargs.get("max_speakers") == 4, f"max_speakers=4 not passed: {captured_kwargs}"
    ...
    def test_return_speaker_embeddings_passed_to_pipeline(self, client):
        ...
        async def _capture_run(*args, **kwargs):
            captured_kwargs.update(kwargs)
            result, embeddings = _mock_pipeline_result_diarized(speaker_embeddings={"SPEAKER_00": [0.1] * 256})
            return result, embeddings
...
class TestGracefulSkipNoToken:
    ...
    def test_no_speakers_when_hf_token_missing(self, client):
        ...
        body = resp.json()
        for seg in body["segments"]:
            assert not seg.get("speaker"), f"Should not have speaker labels without HF_TOKEN: {seg}"
    ...
    def test_no_embeddings_when_hf_token_missing(self, client):
        ...
        assert resp.status_code == 200
        body = resp.json()
        assert "speaker_embeddings" not in body, "speaker_embeddings should not be present without HF_TOKEN"
...
class TestDiarizationWithoutAlignment:
    ...
    def test_speakers_assigned_when_word_timestamps_false(self, client):
        ...
        body = resp.json()
        speakers = {seg.get("speaker") for seg in body["segments"] if seg.get("speaker")}
        assert len(speakers) >= 2, f"Expected >=2 speakers with diarize=true, word_timestamps=false; got: {speakers}"
...
class TestSingleSpeakerDiarization:
    ...
    def test_single_speaker_yields_one_label(self, client):
        ...
        body = resp.json()
        speakers = {seg.get("speaker") for seg in body["segments"] if seg.get("speaker")}
        assert len(speakers) == 1, f"Single-speaker clip should yield 1 speaker label, got: {speakers}"
